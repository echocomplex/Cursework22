...
void cursework::sort (List <Student>& unit) const {
    for (int i = 0; i < unit.length() - 1; ++i) {
        for (int j = 0; j < unit.length() - i - 1; ++j) {
            std::string last_name = unit[j].getLastName();
            std::string next_last_name = unit[j + 1].getLastName();
            if (last_name[0] > next_last_name[0]) {
                Student temp = unit[j];
                unit[j] = unit[j + 1];
                unit[j + 1] = temp;
            }
            else if (last_name[0] == next_last_name[0]) {
                int length = (last_name.size() < next_last_name.size()) ? last_name.size() : next_last_name.size();
                for (int k = 0; k < length; ++k) {
                    if (last_name[k] > next_last_name[k]) {
                        Student temp = unit[j];
                        unit[j] = unit[j + 1];
                        unit[j + 1] = temp;
                    }
